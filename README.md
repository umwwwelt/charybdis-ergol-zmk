...
- `MACOS`
- `HT_TWO_THUMB_KEYS` (mode Selenium 34 touches + 1)
- `VIM_NAVIGATION`
...
- HRM: S/D = CTL/GUI inverse, K/L = GUI/CTL inverse.
...
- `HRM_STAB` = hold `LGUI`, tap `RS(TAB)`; `HRM_TAB` = hold `LALT`, tap `TAB`.
...
- L_HMBSP (VimNav + Backspace) fonctionne maintenant sur NumRow.
- `▽` = transparent (`&trans`).
